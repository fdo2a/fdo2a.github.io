...
## 2026-08-30 재개 — 남은 결함 (codex 검토, 미해결)

브랜치를 5일 만에 되살려 Task 6·7·9·10·11·12를 마치고 codex 검토를 받았다. **계약 수준의
결함 여섯은 고쳤고**(산출 파일 충돌·update_archives 호출 규약·날짜 마스킹 과잉·단위 붙은
작은 정수 면제·시장별 수급 일수 오라벨·스테일 집계 파일 커밋), **아래는 남았다. 트리거를
등록하기 전에 처리해야 한다.**

### 발행 게이트가 창작을 놓치는 자리

- **출처 구분이 없다.** `period_gate`가 집계·스코어카드·발행본의 수치를 한 덩어리로
  합쳐 놓아서, 어느 날 PMI가 `57.3`이었으면 「S&P 500 +57.3%」가 통과한다. 값이 **어느
  항목의 것인지**를 함께 봐야 한다
- **집계의 시작·종료 날짜가 허용 수치로 샌다.** `start_date: 2026-08-17`에서 `-17`이
  허용 집합에 들어가 「-17%」가 통과한다
- **스코어카드 라벨과 값의 짝을 검사하지 않는다.** 「적중」에 엉뚱한 점수를 붙여도 통과
- `span`·`complete`·필수 섹션·거래일 수를 검사하지 않는다

### 복기 스코어카드의 논리

- **구간별 채점이 실제로는 구간별이 아니다.** `period_scorecard.score()`가 기간 전체
  수익률을 모든 구간에 공통 적용해서, 등급이 중간에 뒤집힌 경우 두 판단이 다 맞았어도
  0으로 상쇄된다. 구간별 실현치를 재려면 일별 계열이 필요하다
- **레짐 점검이 `growth == 0`이면 모든 발표와 일치로 나온다.** 그리고 `rows[-1]`을
  기간 종료일로 자르지 않아 과거 기간을 다시 만들면 미래를 본다
- **`trigger_hygiene`이 발동 이력을 보지 않는다.** 같은 트리거 정의가 몇 번 등장했는지만
  세므로, 여러 번 발동한 트리거도 「잠들었다」로 분류될 수 있다
- **롤업이 당기를 넣기 전에 계산된다.** 첫 실행과 재실행의 결과가 다르다
- **월간 `--spans 3,12`가 「3개월·12개월」이 아니라 「주간 3행·12행」이다**

### 파이프라인 정합

- **주간·월간 발행본에 `check_weight.py`를 걸지 못한다.** 그 게이트는 일간 섹션 제목
  (「주식」·「채권」·「매크로 논리」)을 요구해서 5섹션짜리 총정리를 잘못 막는다 — 기간용
  판정이 따로 필요하다
- **오케스트레이터가 AI 티 제거(STEP 2.5) 단계를 건너뛴다.** 일간은 `humanize_prose.py`
  경로를 지나는데 기간 리포트는 조판·문체 게이트만 돌고 바로 발행한다
- **월간 롤오버 판정이 「달이 닫혔는가」가 아니다.** 집계 파일 존재와 HTML 부재만 보므로
  새 달 이틀째에 1세션짜리 당월 집계로 발행될 수 있다
- **주간 URL이 설계와 다르다.** 설계는 커버 기간 마지막 거래일 기준인데 구현은 ISO 주
  키(`weekly/2026-W35.html`)를 쓴다. 둘 중 하나로 통일해야 한다
- **`data/history/*.jsonl`이 08-21에서 멈춰 있다.** `stance.json`은 08-28인데 이력이
  안 따라와서 그 기간 등급 변경을 구간으로 못 나눈다 — `backfill_history.py`를 돌려야 한다
- **KR 수집기의 기간 집계가 시세를 다시 받는다.** 「재수집 금지」 원칙과 어긋난다(US도
  같다). 총정리 수치가 이미 나간 일간과 갈릴 수 있다

### 테스트

`test_period*.py` 89개가 위 결함을 **전부 통과시킨다.** 헬퍼 단위 검사이지 발행 계약
검사가 아니다. 위를 고칠 때 실제 발행본·실제 CLI 호출을 대상으로 하는 검사를 함께 넣는다.
