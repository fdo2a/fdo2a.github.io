...
원장으로 바꾸면서 **재수집 시절엔 있던 안전장치가 하나 사라진 것**을 codex가 짚었다.
재수집은 설정된 이름을 늘 전부 내놓아(계열이 비더라도) 빠진 종목이 `missing`에 걸렸지만,
원장은 없는 이름을 아예 안 만든다. 그래서 셋을 넣었다.

- **명부를 이력 전체의 합집합으로 세운다**(`series_from`). 어제까지 있던 종목이 오늘
  사라지면 빈 계열로 남아 `missing`에 걸린다
- **끝값이 기간 마지막 거래일의 값인지 검사한다**(US `build`, KR `finalize`). 이것이
  「성과표 끝값 = 그 기간 마지막 발행본 종가」를 실제로 강제하는 검사다 — 마지막 날
  값이 없으면 끝값이 하루 전 값이 되어 조용히 갈린다
- **KR 지수 원장이 통째로 비면 `complete: false`.** 이름이 없으면 검사 루프가 한 번도
  안 돌아 빈 지수 칸이 완성본으로 나갔다

**같은 날 정정본을 받는다** — `append_jsonl(upsert=True)`. 수집 워크플로는 하루 두 번
돌고 발행본은 마지막 실행의 스냅샷으로 나가는데, 선착순이면 원장이 첫 실행 값을 들고
발행본과 갈린다. 판단 이력(stance·macro)은 선착순 그대로다.

게이트도 둘 더. **bp를 레벨과 다른 칸에 둔다** — 「7711.76bp」가 종가와 대조돼 통과하던
자리다. **메모리·AI 인프라·금리도 항목 검사 대상**이 됐다(금리는 한국어 이름 「10년물」로
잡는다 — `10Y`로는 본문에서 한 번도 안 걸렸다).

테스트 991개 통과(신규 30개).

### 여전히 남은 것

codex가 남긴 것 중 손대지 않은 것: `_check_provenance`의 이름 부분일치 오탐(「S&P 500
Growth」), 그리고 원장에 **한 번도 등장한 적 없는** 종목은 여전히 아무도 모른다는 것
(명부의 출처가 원장뿐이라 원리적으로 그렇다 — 수집기 쪽 설정과 대조해야 풀린다).

그 밖에 위 「함께 발견한 것(미해결)」 목록은 그대로다 — 레짐 채점의 `released` 필드 불일치,
