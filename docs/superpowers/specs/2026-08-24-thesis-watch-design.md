...
```
Actions 17:40 KST  →  thesis/data/{watch.json, history.jsonl} + thesis/*.html 재렌더
                                    ↓
...
한 줄 = `{"date": "...", "tickers": {티커: {eps_fy1, eps_fy1_low, eps_fy1_high, price,
pb, price_date, band1, band2, bear}}}`.

뒤의 네 개는 2026-08-25에 추가했다. 교차 판정이 «그날 그은 선»을 필요로 하는데, 선은
매일 추정치로 다시 계산되므로 나중에는 복원할 수 없다. 수집기가 이미 갖고 있는 값이라
네트워크 호출은 늘지 않는다.
...
| 키 | 조건 | 산출 |
|---|---|---|
| `consensus_swing` | FY1 EPS 30일 변화 ≥ ±20% (15% 아래로 진정돼야 재장전) | 밸류 재계산 + 알림 |
| `consensus_floor` | FY1 EPS avg가 low 대비 +15% 이내로 **오늘 내려앉았고**, 평균이 실제로 하락 | kill 후보 |
| `bear_proximity` | 주가가 bear FV ±10% 안으로 **오늘 들어섰거나** 더 내려감 | 주의 |
| `bear_exit` | 주가가 bear 권역에서 **오늘 위로 올라옴** | 정보 |
| `dispersion_widening` | high/low 비율 30일 대비 +30% (20% 아래로 좁혀져야 재장전) | 주의 |

### 조건은 «사건»이어야 한다 — 2026-08-25 수정

첫 판은 `band_entry`·`bear_proximity`·`consensus_floor` 셋을 **수준 검사**로 썼다.
「지금 선 아래인가」를 물었으므로 조건이 유지되는 내내 매일 발화했다. 실전 이틀째에
삼성전자가 1차 관심선 아래로 내려가면서 드러났다 — STEP 3의 「트리거도 사건도 없으면
아무것도 하지 않는다」가 삼성에 대해서는 영영 닫혔고, 매일 «별일 아님»을 에이전트의
판단으로 증명해야 했다. 판단을 없애려고 만든 파이프라인에서 판단이 매일 재생산된 것이다.

그래서 남은 둘을 **교차**로 바꿨다. 상태가 어제와 달라진 날에만 울리고, 유지되는 동안은
침묵한다. 기준선은 `history.jsonl`의 «오늘 이전 마지막 관측»(`history.previous`)이고,
비교 없이는(종목 첫날, 옛 행) 발화하지 않는다.

`band_entry`는 **트리거에서 뺐다.** 주가가 관심선 아래에 있는 것은 논거가 흔들린 것이
아니라 싸진 것이고, 오케스트레이터도 「단기 주가 하락만으로는 알림 대상 아님」이라고
이미 못박아 뒀다. 위치는 `position.in_band1`·`in_band2`가 상시 들고 있고 `render.py`가
페이지에 「1차 관심선 아래」로 표시한다 — 상태는 보이되 종은 울리지 않는다.

`consensus_floor`에는 **방향 조건**이 붙었다. 평균이 내려와 하단에 가까워진 것만 약세
수렴이다. 하단 추정치가 올라와서 좁혀진 것은 오히려 비관론이 걷힌 것인데, 첫 판은 그
둘을 구분하지 못하고 똑같이 최고 등급(kill 후보)으로 불렀다.

교차는 각 날을 **그날의 선**과 견준다. 추정치가 무너져 bear 값이 정체된 주가까지 올라온
날도 상태 변화이므로 한 번 발화한다. 무엇이 움직였는지는 **어제 선을 그 자리에 고정한 채
오늘 주가만 넣어 보고** 가른다 — 그래도 상태가 달라지면 움직인 것은 주가고, 그대로면
움직인 것은 선이다. 가격이 같은지만 보면 「주가는 올랐는데 기준선이 더 빨리 올라온」 날을
주가 탓으로 돌리게 된다. 그래서 `history.jsonl`에 그날의 `band1`·`band2`·`bear`를 남긴다.

**회복도 사건이다.** 처음엔 악화 방향만 울리게 했는데, `state.propose()`가 트리거 없이는
등급을 못 움직이므로 그러면 내려간 등급이 영영 못 올라온다. bear 권역에서 위로 벗어난
날은 `bear_exit`(정보)로 울린다.

이 수정은 codex와 상의해 나왔다. 「매일 발화한다」는 진단은 맞았지만 처방은 codex 쪽이
나았다 — 셋을 일괄 교차로 바꾸려던 것을, 성격에 따라 «상태값으로 강등 / 교차 / 방향
보강 후 교차»로 갈랐다. 구현본을 다시 읽혀 회복 방향을 악재로 부르던 것, `in→below`가
잘못된 라벨을 달던 것, bear가 음수일 때 띠가 뒤집히던 것, 원인 판정이 가격 일치 비교라
틀리던 것을 잡았다.

### 남은 것

- 기록을 넓히기 전 행(2026-08-24)이 어제인 하루 동안은 bear 교차를 판정할 수 없다.
  그날은 침묵한다 — 추측하지 않는다.
### 창이 굴러가는 지표는 재장전이 필요하다 — 2026-08-25

`consensus_swing`·`dispersion_widening`은 30일 전과 비교하는 지표다. 창이 굴러가는 동안
값이 임계 위에 며칠씩 머물므로, 「지금 20%를 넘었나」로 물으면 하나의 사건이 며칠 연속
보고된다. 그래서 한 번 울리면 **충분히 진정될 때까지** 다시 울리지 않게 했다. 발화선과
재장전선을 떼어 놓은 것(20%→15%, 30%→20%)은 임계선 근처에서 값이 오르내릴 때 매일
재발화하는 것을 막기 위해서다 — 20%를 넘었다 18%로 내려온 것은 진정된 게 아니다.
방향이 뒤집히면(상향 국면 → 하향) 같은 국면이 아니므로 바로 울린다.

상태 파일은 두지 않는다. `prior_metrics()`가 기록을 다시 훑어 계산하므로 백필하거나 다시
돌려도 결과가 같고, 조용한 날에 파일을 건드리지 않는다는 규율도 지킨다.

### 페이지는 매일 다시 렌더한다 — 2026-08-25

페이지가 트리거 있는 날에만 다시 만들어져서, 2026-08-25에 삼성 페이지가 사흘 낡은
281,500원·「관심선 위」를 띄우고 있었다(실제 257,000원·1차선 아래). `band_entry`를 뺀
뒤로는 관심선 진입만으로 다시 만들어질 일도 없어졌다.

그래서 **Actions 수집 단계가 매일 페이지를 다시 렌더해 커밋**한다. 산문은 `content.py`,
수치는 방금 커밋한 `watch.json`에서 오므로 판단이 들어갈 자리가 없다. 등급과 산문은
여전히 루틴만 바꾼다.

발행 후 검토 게이트와는 이렇게 갈랐다. 페이지가 매일 바뀌면 매일 네 건이 검토 큐에
오르는데, 바뀐 것은 숫자뿐이라 읽을 것이 없다. 그래서 **게이트는 `/thesis/` 페이지를
보지 않고, 그 페이지가 렌더되는 원고(`scripts/thesis/content.py`·`narrative.py`)를
본다.** 사람이 쓴 것이 그쪽이다.

손으로 페이지를 고치는 경우는 `build_thesis_pages.py --check`가 막는다. 워크플로가
**수집 전에** 돌려서, 커밋된 페이지가 커밋된 데이터로 렌더한 결과와 같은지 본다. 순서가
핵심이다 — 수집 후에 대조하면 그날의 정상적인 시세 변화까지 「다르다」로 나오고, 렌더를
먼저 돌리면 손편집을 잡는 게 아니라 덮어써 버린다. 다르면 데이터는 그대로 커밋하되 그
실행을 실패로 남긴다. 「HTML 직접 편집 금지」가 규칙이었는데 지켜졌는지 확인할 방법이
없었다.

한계 두 가지. `watch.json`에 없는 티커(수집 실패분)는 렌더도 대조도 건너뛰므로 그 종목
페이지는 낡은 채로 남는다. 그리고 빌더가 만드는 목록에 없는 파일을 `/thesis/`에 새로
넣으면 아무도 보지 않는다.

부수적으로 `narrative.html`의 수정일이 시세의 `as_of`를 쓰고 있어 산문이 그대로인 날에도
매일 새 파일이 됐다. 실제로 산문이 바뀐 날(`narrative.UPDATED`)을 쓴다.
