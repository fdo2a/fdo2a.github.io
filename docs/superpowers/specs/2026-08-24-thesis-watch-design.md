...
- `consensus_swing`·`dispersion_widening`은 아직 수준 검사다. 30일 창의 값이 임계를
  넘는 동안 매일 발화한다. history가 20행에 닿는 2026-09-18경부터 실제로 문제가 된다.
- 기록을 넓히기 전 행(2026-08-24)이 어제인 하루 동안은 bear 교차를 판정할 수 없다.
  그날은 침묵한다 — 추측하지 않는다.
- **발행 페이지가 트리거 없이는 갱신되지 않는다.** 상태값을 페이지가 보여준다는 전제가
  실제로는 마지막 빌드 시점에 멈춘다(2026-08-25 확인: 페이지는 「관심선 위 281,500원」,
  데이터는 257,000원·1차선 아래). 수집 단계에서 페이지를 다시 렌더하는 것이 자연스럽지만
  발행 후 검토 게이트와 얽히므로 별도 판단이 필요하다.
