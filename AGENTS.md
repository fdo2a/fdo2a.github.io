<!-- This file is where shared rules live. codex walks project docs from the git repo
     ...
     This file + one leaf (scripts/*/CLAUDE.md) must stay under 32,768 B — past that
     codex drops the tail **with no marker**. Raise it with `project_doc_max_bytes = 65536`
     in `~/.codex/config.toml` (verified). **This file must stay under 13,000 B**: growth
     here truncates every leaf at once, so the ceiling is enforced on its own rather than
     only as part of the chain. Gate: scripts/common/tests/test_agent_docs.py -->
...
- **Tests**: `cd site && python3 -m pytest scripts` (measured 2026-09-06: 1,762 passed in 15 s — whole tree, no `--ignore`). **Must run from `site/`.** From the project root it fails collection (`ModuleNotFoundError: scripts`) and ends with "N collected, 6 errors" — **the count alone looks like a pass, so read the `Interrupted` line.** The dependency is the `sys.path` insert in `scripts/conftest.py`; there is no ini config.
...
- **Check unreviewed posts**: `python3 scripts/review_gate.py pending --hook`. The SessionStart hook calls this automatically, but the hook ends in `|| true` and fails silently — run it by hand whenever the queue looks wrong. "typography-only: N" (a bulk `apply_readability.py` run moved many SHAs) clears with `review_gate.py refresh` then `--apply`. "판정 불가" means the block is missing from `scripts/review/known_blocks.py` or the base blob was not found — inspect by eye.

## Adding something new — keep this shape

Before you write new guidance, **decide where it goes.** `scripts/common/tests/test_agent_docs.py` (31 tests) enforces the shape.
...
4. **This file ≤ 13,000 B, and this file + one leaf ≤ 32,768 B.** Past that codex discards the tail **with no marker**. When the test fails, do not squeeze the leaf — move history into a spec. **Growth here truncates every leaf at once**, so before writing here, suspect first that it belongs to a single pipeline.
...
Design rationale and incident history live in `docs/superpowers/specs/`. Each leaf's **"When changing this"** line names the spec to read.
...
5. **Voice — write the way you talk** (Korean prose): name the subject, use active voice, one relation per sentence, one topic per paragraph (2–4 sentences). No body indent; `word-break: keep-all`. What to filter is not loanwords but **words the reader is seeing for the first time** — ① terms that always appear in market coverage stay as they are (`COMMON`); ② only transliterations that fail that bar get spelled out (`PLAIN`); ③ jargon that not even the news uses gets glossed once on first appearance (`GLOSS`); ④ **never stack unfamiliar words in one sentence** (only ② and ③ count). **The three vocabulary groups and the stacking limit live in `scripts/us/style.py`** (`COMMON`, `PLAIN`, `GLOSS` + the per-sentence cap), and `scripts/check_style.py` checks all four rules (path argument only, no `--html`). Tables, headings, and captions are exempt. Details: `.claude/agents/brief-report-writer.md`, section 「말하듯이 쓴다」.
   **STEP 2.5 "remove the AI tell", just before publishing**: never touch the original — edit a `*.humanizing.html` copy. Run `humanize-korean:humanize-korean` as the final editing pass on new Korean prose (facts, numbers, dates, proper nouns and quotes unchanged; style only), then move the prose from `_workspace/{run_id}/final.md` **into the authored source, not the generated HTML**. Do the swap with **`scripts/humanize_prose.py finalize`**, never by hand — it reinserts by label rather than by counting paragraphs, rejects per-paragraph changes to numbers, tickers, and judgment vocabulary as well as reordered paragraphs, and **replaces the original only when everything passes**. **Never `mv` by hand.** Logic: `scripts/us/prose_swap.py`.
   **A readability warning rejects that draft, not the day's report** — fix only the flagged passages and rerun.
...
9. **Verification**:
   ⓪ **A `<div>` crossing a section boundary is caught by a gate** — even when open/close counts balance across the whole document, a div still open at `</section>` closes the body container there, so later sections lose the 1120px cap and the side margins (measured 2026-09-03). `readability.section_div_breaks()`, called by `check_readability.py` before both US and KR publishing.
...
There are two review points because a wrong design wastes the whole implementation. **This does not mean asking for approval again.** Cloud routine sessions have no codex CLI, so this is a local-session rule. Delegate to codex also when the same problem has failed 2–3 times.
