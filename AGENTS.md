...
5. **Voice — write the way you talk** (Korean prose): clarify actors, use active voice, one relation per sentence, one topic per paragraph. No body indent; `word-break: keep-all`. Filter only **words the reader is seeing for the first time** — `COMMON` terms stay, `PLAIN` transliterations get spelled out, `GLOSS` jargon is glossed once, and **never stack them in one sentence** (lists in `scripts/us/style.py`). **US/KR (daily, weekly, monthly) are desk documents for a PM** declared by `<body data-register="da">`: `-다` register only, no gloss duty, no rhetorical Q&A, no workflow words (원장·회차·판정불가, filenames, code) even in captions, calculations inside `<details data-provenance>`. `scripts/check_style.py` checks all of it (path only, no `--html`; warnings exit 0). Details: `.claude/agents/STYLE_EXEMPLARS.md` §5, `brief-report-writer.md` 「말하듯이 쓴다」.
