...
# 며칠 전 문서까지 훑을 것인가. 오래된 문서를 오늘 것처럼 싣기 위한 폭이 아니다 —
# 실제 발행 여부는 `first_seen` 이 정하고, `seen.json` 이 같은 문서를 두 번 싣지
# 못하게 한다. 그래서 창은 **놓친 것을 되찾는 폭**이고, 넓혀도 중복이 안 생긴다.
#
# 2026-09-03 에 4일에서 10일로 넓혔다. 4일은 주말만 겨우 건너뛰는 폭이라, 수집이
# 며칠 실패하면 FOMC 성명이나 잭슨홀 연설이 **영영 사라졌다**. 연준 tier 1 자리는
# 하루 지났다고 없던 일이 되지 않으므로 되찾을 수 있어야 한다. 대신 지난 날짜
# 이벤트는 발행본이 그 날짜를 밝혀야 한다(게이트가 검사한다).
WINDOW_DAYS = 10
