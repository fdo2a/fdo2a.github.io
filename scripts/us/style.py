...
# ── 쉬운 말 (2026-08-26 사용자 지시: 「브레드스 확산 트리거같은 뭔 소린지 모르는
# 용어 사용은 자제하고 최대한 풀어서 작성해」) ────────────────────────────────
#
# 두 부류를 갈라 다르게 다룬다.
#
# PLAIN — 한국어 대응이 이미 있는 음차. 음차할 이유가 없으니 한 번만 나와도 잡고,
#         바꿔 쓸 말을 함께 찍어 준다.
# GLOSS — 뜻을 옮기면 길어지는 진짜 전문어. 쓰는 것은 되지만 **첫 등장에서 한 번은
#         풀어야** 한다. 괄호든 「즉/다시 말해」든 「~라는 뜻」이든 형식은 안 따진다.
#
# 실측(발행본 52편 산문): breadth 212·트리거 190·컨센서스 169·밸류에이션 153.
...
PLAIN = {
    ...
    '아웃퍼폼': '시장보다 더 오름', '언더퍼폼': '시장보다 덜 오름',
    '오버웨이트': '비중 확대', '언더웨이트': '비중 축소',
    '스티프닝': '장단기 금리차 확대', '플래트닝': '장단기 금리차 축소',
    '인버전': '장단기 금리 역전', '디스인버전': '금리 역전 해소',
    '숏커버': '공매도 되사기', '디레버리징': '빚 줄이기',
    '리레이팅': '몸값 재평가', '디레이팅': '몸값 하향',
    '익스포저': '노출·투자 비중', '포지셔닝': '자금이 어디에 실렸나',
    '리스크온': '위험자산 선호', '리스크오프': '안전자산 선호',
    '디커플링': '따로 움직임', '오버슈팅': '과하게 튐', '바텀아웃': '바닥 확인',
    '캐치업': '뒤따라 오름', '롤다운': '만기가 짧아지며 붙는 이익',
    '컨빅션': '확신', '프록시': '대리 지표', '모멘텀': '가격 흐름의 힘',
}
# 뜻을 옮기면 문장이 무거워지는 것들. 첫 등장에서 한 번 풀면 그 뒤로는 그냥 쓴다.
GLOSS = (
    '기간프리미엄', '기대인플레', '디스인플레이션', '확산지수', '레짐',
    '밸류에이션', '컨센서스', '가이던스', '멀티플', '캐리', '베타',
    # 파이프라인 내부 말이지만 발행본에도 그대로 나간다. 사용자가 든 예가
    # 「브레드스 확산 트리거」였으니 트리거도 첫 등장에서 한 번은 푼다.
    '트리거', '스탠스',
)
...
# 뒤에 뭐가 붙든 대개는 여전히 그 말이다 — 「밸류에이션발」·「숏커버성」·
# 「캐리트레이드」는 실측 발행본에 있고, 전부 잡아야 맞다. 조사 목록을 허용
# 집합으로 두는 방식은 이것들을 놓치고 「베타입자」를 잘못 잡았다(codex 검토).
# 그래서 **뒤는 열어 두고**, 그 말이 아닌 합성어만 예외로 적는다. 짧고 눈으로
# 검토되는 목록이라 새 오탐이 나오면 여기 한 줄 더한다.
NOT_JARGON = ('베타테스트', '베타버전', '베타서비스', '알파베타')
