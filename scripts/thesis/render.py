...
def valuation_block(row, as_of):
  ...
  시나리오 P/B)을 각각 계산해 평균했다. 두 방법은 같은 이익 전망에 영향을 받으므로
  결과가 비슷하다는 이유만으로 독립적인 검증을 거쳤다고 볼 수 없다.</p>
...
{rows}
  ...
  측정이 아니라 판단이다. 확률과 배수를 바꾸면 가치와 현재가의 관계도 달라질 수 있다.
  이 값은 목표주가가 아니라 가정별 판단 기준이며, 관심선 진입만으로 거래를 결정하지 않는다.</p>
</section>'''
...
def desk_block(row, book, as_of):
    """Decision context from the existing snapshot and open-question book, no new thesis state."""
    expectation = (
        'FY1 컨센서스는 분석가들의 이익 추정치다. 투자자 포지셔닝이나 가격에 얼마나 반영됐는지를 '
        '직접 보여주지는 않는다. 추정치의 변화와 실제 실적을 함께 확인해야 한다.'
        if row.get('eps_fy1') is not None else
        '컨센서스 자료가 없어 현재 이익 기대와의 차이를 판단할 수 없다.'
    )
    questions = book.get('open_questions') or []
    pending = ('<ul>' + ''.join(f'<li>{escape(str(q))}</li>' for q in questions) + '</ul>'
               if questions else '<p>기록된 미해결 질문이 없다. 새 근거 없이 새 판단을 덧붙이지 않는다.</p>')
    return f'''<section data-block="desk" data-as-of="{escape(str(as_of), quote=True)}">
  <h3>운용 판단에 연결할 쟁점</h3>
  <p>{expectation}</p>
  <p>아래 투자 논거와 촉매는 검증할 가설이다. 반증 조건이 충족되는지 함께 확인하고,
  이전 판단을 바꾼 근거는 변경 이력의 사실·추론·변화·다음 확인 사항을 따라 읽는다.
  기록이 없는 판단이나 수익률을 사후에 복원하지 않는다.</p>
  <h4>다음 확인 사항</h4>
  {pending}
  <p class="caveat">보유 여부와 투자 기간은 가정하지 않는다. 등급은 기존 논거의 상태를 나타내며
  주문 지시가 아니다. 고객 주문·딜러 재고·실시간 호가·실행 비용·차입 가능 여부는 미확인이다.
  현물과 파생상품의 유동성, 환율 노출, 보유 기간을 확인하기 전에는 실행 가능성을 확정하지 않는다.</p>
</section>'''
