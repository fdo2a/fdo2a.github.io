...
1. **이번 회차의 「오늘(KST)」을 파일에 박는다.** 루틴은 UTC 에서 돈다 — 머리로 「오늘」을
   ...
   **파일 하나가 회차 전체의 「오늘」이다** — 아래 어디서도 날짜를 다시 계산하지 않는다.
   ...
   이제 `china/data/releases/index.json` 의 신선도를 그 날짜와 견준다. **`generated` 는 2026-09-25
   부터 KST(`+09:00`)로 찍지만, 그 전 파일은 UTC 라 날짜 부분을 그대로 비교하지 않는다** — `2026-09-12T16:31Z` 는 KST 로 09-13 이다. 아래 명령은 둘 다 KST 로 바꿔 본다.
   판정까지 한 명령에서 끝낸다:
...
3. **발행이 수집보다 먼저 돈다.** 발행은 11:00 KST, 수집 cron 은 15:00 KST 인데 스케줄러가
   몇 시간씩 밀어 실제로는 19~20시 KST 에 돈다. 그래서 회차 아침의 `generated` 는 **언제나
