...
승계되는 두 섹션(§9 매크로 논리 · §9 멀티에셋 스탠스)은 추가 입력을 읽는다 — `macro.json` / `macro_eval.json` / `macro_metrics.json`, `stance.json` / `stance_eval.json` / `stance_metrics.json`. 이들은 **비-코어**라 없으면 각 섹션의 부트스트랩·동결 규칙을 따르되, 있으면 그 판정이 계약이다. 산출물도 HTML 하나가 아니라 **`macro_next.json`·`stance_next.json`까지 셋**이다.
...
9. **매크로 논리** — 레짐 + 4축 진단(**축별 판정 문단 바로 아래 그 축의 지표 표**) + 정책 경로 + 자산별 전달경로 + 시장 해석·다음 발표 일정 카드. **아래 상세 사양을 반드시 따른다. §10과 마찬가지로 그날 데이터로 새로 쓰는 것이 아니라 전일 판단을 승계한다.** 과거 맨 뒤 §13에 따로 있던 경제지표 대시보드는 2026-08-20 사용자 지시로 이 섹션에 흡수됐다 — 판정과 그 근거 숫자가 같은 자리에서 읽힌다.
...
11. **멀티에셋 매니저 전략** — 전일 스탠스 리뷰 + 스탠스 표(주식/채권/FX/원자재/메모리/AI 인프라) + 리스크 시나리오 2-3개. **아래 상세 사양을 반드시 따른다. 이 섹션만은 그날 데이터로 새로 쓰는 것이 아니라 전일 포지션을 승계한다.** (이 문서 다른 곳에서 「§10 스탠스」로 부르는 그 섹션이다.)
12. **모의 포트폴리오** — 등급이 실제로 얼마의 비중이 되고 그래서 얼마를 벌었는지. **아래 상세 사양을 반드시 따른다. 수치는 전부 `portfolio.json`에 계산돼 있다 — 직접 계산하지 말 것.**
13. **주목 섹터·종목** — top 2-3 movers
14. **메모리/DRAM** — 표 + 업계 뉴스 + 투자 관점
15. **AI 인프라** — 표에 분야 컬럼 + 업계 동향 + 투자 관점
16. **오늘의 학습과 복기** — `.claude/TRADER_LEARNING.md`의 Writer output을 따른다. 이 섹션 뒤에 면책 문구를 붙인다.
...
### 섹션 9. 매크로 논리 — 상세 사양 (2026-08-18 사용자 지시)
...
### 섹션 11. 멀티에셋 매니저 전략 — 상세 사양 (2026-08-17 사용자 지시)
...
### 섹션 12. 모의 포트폴리오 — 상세 사양 (2026-09-01 사용자 지시)

사용자 지시: 「멀티에셋전략 파트에 말만 그렇게 하지 말고 실제 포트폴리오를 운영하고 성과까지 한번 공유해봐.」

멀티에셋 전략 표는 **방향**만 말했다. 「주식 소폭확대」가 몇 퍼센트인지, 그래서 지금까지 돈을 벌었는지를 발행본이 한 번도 말하지 않았다. 이 섹션이 그 자리를 메운다. 바로 앞 섹션 뒤에 붙인다.
