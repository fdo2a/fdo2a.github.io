...
**⑤-c 팩터 분해 (`factor_decomposition`)** — 메모리·AI 인프라가 시장을 앞서거나 뒤진 것이 **그 산업 얘기인지 스타일 탓인지** 가른다. 몫은 넷이다: 시장 민감도 · 성장−가치 · 대형−소형 · 고유 요인. §11·§12의 해석 문단에서 쓴다. **`beta`는 인쇄하지 않고 `contribution`만 쓴다.** 네 몫을 더하면 `excess_pct`가 되며, 그 값은 **하루치 초과수익을 20거래일 더한 것**이라 §9 표의 20일 초과수익(20거래일 전 종가와 직접 견준 값)과 기준이 다르다 — 둘을 나란히 쓸 때는 그 차이를 캡션에 밝힌다.
