...
§4 주식 · §6 채권 · §7 FX · §8 원자재는 각각 `<p data-standing="...">` 문단으로 시작한다(값은 `equities`·`bonds`·`fx`·`commodities` 중 하나). **120자 이상이고 수치를 하나 이상** 담는다.
...
### 문체 정본 = 「모의 포트폴리오」 섹션 (2026-09-01 사용자 지시 「문체는 이걸 참고해 … 이제부터 그렇게」)

아래 「말하듯이 쓴다」의 기준을 실제 문장으로 보여 주는 표본이 2026-08-31 발행본의 **모의 포트폴리오 섹션**이다. 새 섹션을 쓸 때 그 문장들을 먼저 읽고 맞춘다. 뽑아 쓸 수 있는 특징은 다섯이다.

1. **문단은 2~3문장.** 길어지면 나눈다. 한 문단 = 한 주제. 다만 **그 문단들은 서로 이어져야 한다** — 아래 「하나의 글로 읽히게 쓴다」가 이 조건이고, 2026-09-13 지시로 덧붙었다.
2. **나쁜 결과를 먼저, 흐리지 않고 쓴다.** 「등급을 얹어서 얻은 것은 -0.35%포인트, 아직 손해입니다.」 — 초과수익이 마이너스인 날 그것을 완충하는 문장을 앞에 놓지 않는다.
3. **한계를 그 자리에서 밝힌다.** 「거래비용은 반영하지 않았습니다.」 · 「기록이 아직 10거래일뿐이라 …표본이 모자랍니다.」 — 각주로 미루지 않는다.
4. **규칙에는 이유를 한 문장 붙인다.** 「마감 뒤에 정한 것을 그날 종가에 담으면 결과를 미리 보고 산 셈이 되기 때문입니다.」 — 규칙만 적으면 독자가 외워야 하고, 이유가 붙으면 이해한다.
5. **정확한 값은 표, 산문은 관계와 원인.** 「어디서 까먹었는지는 분명합니다」로 열고 기여도 셋을 나열한 뒤 왜 그랬는지로 잇는다.
...
- **문장은 여전히 120자 이하다.** 줄글은 문장을 늘려서 만드는 게 아니라 문단을 이어서 만든다. 게이트 상한은 120자 경고·160자 실패, 문장당 수치 4개 경고·6개 실패다. 이어 붙이려다 문장이 길어지면 게이트가 막는다.
- **긴 문단을 쓰려면 `<body data-layout="prose">` 를 단다.** 이 표시가 없으면 `apply_readability.py` 의 `split_dense_paragraphs` 가 320자 넘는 3문장 이상 문단을 **두 문장씩 잘라** 도로 토막글로 만든다. 인라인 마크업이 있는 문단은 건너뛰므로, 표시가 없던 시절에는 굵은 리드를 단 문단만 살아남았다 — 조판기가 토막글을 보상하고 줄글을 벌한 셈이고, 위 26/28 이 그 적응 결과다.

**일간 §2 는 예외적으로 골격이 고정이다.** `check_weight.py` 가 `data-lede="event" → "meaning" → "action" → "invalidation"` 네 문단을 순서대로 요구한다. 네 문단은 유지하되, 각각을 독립 선언으로 쓰지 말고 event 가 만든 상황을 meaning 이 받고 action 이 그에 답하고 invalidation 이 그 답을 뒤집는 조건을 말하는 **한 줄기**로 쓴다.
