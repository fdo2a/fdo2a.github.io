...
9. **매크로 논리** — 레짐 + 4축 진단(**축별 판정 문단 바로 아래 그 축의 지표 표**) + 정책 경로 + 자산별 전달경로 + 시장 해석·다음 발표 일정 카드. **아래 상세 사양을 반드시 따른다. §9와 마찬가지로 그날 데이터로 새로 쓰는 것이 아니라 전일 판단을 승계한다.** 과거 맨 뒤 §13에 따로 있던 경제지표 대시보드는 2026-08-20 사용자 지시로 이 섹션에 흡수됐다 — 판정과 그 근거 숫자가 같은 자리에서 읽힌다.
