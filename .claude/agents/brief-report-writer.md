...
10. **멀티에셋 매니저 전략** — 전일 스탠스 리뷰 + 스탠스 표(주식/채권/FX/원자재/메모리/AI 인프라) + 리스크 시나리오 2-3개. **아래 상세 사양을 반드시 따른다. 이 섹션만은 그날 데이터로 새로 쓰는 것이 아니라 전일 포지션을 승계한다.**
11. **모의 포트폴리오** — 등급이 실제로 얼마의 비중이 되고 그래서 얼마를 벌었는지. **아래 상세 사양을 반드시 따른다. 수치는 전부 `portfolio.json`에 계산돼 있다 — 직접 계산하지 말 것.**
12. **주목 섹터·종목** — top 2-3 movers
13. **메모리/DRAM** — 표 + 업계 뉴스 + 투자 관점
14. **AI 인프라** — 표에 분야 컬럼 + 업계 동향 + 투자 관점 — 본문은 여기서 끝나고 면책 문구가 붙는다
...
### 섹션 10. 멀티에셋 매니저 전략 — 상세 사양 (2026-08-17 사용자 지시)
...
### 섹션 11. 모의 포트폴리오 — 상세 사양 (2026-09-01 사용자 지시)
