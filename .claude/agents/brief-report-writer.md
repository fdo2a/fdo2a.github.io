...
- **위치** — `price_context.levels`의 **`plain.text`를 그대로 쓴다**. 「최근 2년(504거래일) 가운데 이보다 높았던 날은 20일뿐」처럼 **세어 볼 수 있는 말**이 이미 만들어져 내려온다(`plain.short`는 기간 이름을 뗀 짧은 꼴 — 표 칸이나 두 번째 자산에). **`percentile` 값을 옮겨 적지 않는다** — 「96.4 백분위」는 계산에 맞는 말일 뿐이고, 그걸 읽고 그림이 그려지는 독자는 이 리포트가 상정한 독자가 아니다(2026-09-02 사용자 지적 「그렇게 쓰면 아무도 못 알아들을 것 같다」). `check_style`이 「백분위」를 잡고, 채권 게이트는 발행을 막는다. 축 점수 z를 만지지 않는 것과 같은 계약이다 — **판정은 데이터가 끝내고 writer는 읽어준다**. 다만 그 문장이 무슨 뜻인지는 알고 써야 한다: 위치는 「이보다 높았던 날이 몇 날」이지 고저 폭의 가운데가 아니다
...
**④ 시장 응집도 (`cohesion`)** — 10개 자산이 얼마나 한 덩어리로 움직이는지. `top1_pct`가 높을수록 이름만 다른 같은 베팅이고, 분산 효과가 사라진 상태다. §8 레짐·§10 스탠스와 **어긋나도 되지만 근거로 쓸 때는 수치를 캡션에 밝힌다.** 「상위 1개 공통 요인」처럼 풀어 쓰고 주성분·고유값 같은 말은 쓰지 않는다.

**⑤ 5년 뒤 5년 금리 (`forward_5y5y`)** — §6에서 만기별 등락을 말한 뒤 한 문단. 「10년물이 몇 bp 움직였다」에는 앞 5년의 잡음이 섞여 있고, 이 값은 그걸 걷어낸 뒤쪽 5년이라 성장·물가 전망 변화가 여기서 보인다. `nominal`·`real`과 `date`(두 다리의 공통 기준일)를 함께 밝힌다. **`breakeven_gap_bp`가 15bp를 넘으면 커브 하나가 낡은 것이므로 그날은 이 문단을 생략한다.**
