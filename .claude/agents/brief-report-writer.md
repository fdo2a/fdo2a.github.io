...
§4 주식 · §6 채권 · §7 FX · §8 원자재는 각각 `<p data-standing="...">` 문단으로 시작한다(값은 `equities`·`bonds`·`fx`·`commodities` 중 하나). **120자 이상이고 수치를 하나 이상** 담으며, 세 층을 쓴다.

- **위치** — `price_context.levels`의 `percentile`·`band`. `sessions`가 504 미만이면 「2년」이라 부르지 말고 그 기간으로 정확히 부른다. 백분위는 「위에서 몇 % 안」처럼 우리말로 풀어 쓴다
- **경로** — 1주·1개월 변화, 어떻게 여기까지 왔나. 채권은 이번 사이클에서 이 레벨까지 온 경위
- **오늘의 크기** — `moves.multiple`·`band`. **「미미」면 미미하다고 쓴다** — 작은 움직임을 큰 사건처럼 서술하지 않는 것이 이 층의 목적이다
