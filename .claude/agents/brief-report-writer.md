...
- **어려운 말은 풀어 쓴다 (2026-08-26 사용자 지시 — 「브레드스 확산 트리거같은 뭔 소린지 모르는 용어 사용은 자제하고 최대한 풀어서 작성해」).** 발행본 52편 산문 실측에서 breadth 212회·트리거 190회·컨센서스 169회·밸류에이션 153회가 나왔다. 독자는 개인이고, 이 말들을 아는 사람만 읽으라는 리포트가 아니다. 세 갈래로 다룬다.

  1. **한국어 대응이 있는 음차는 그냥 쓰지 않는다.** 브레드스/breadth → 상승 종목 비율, 아웃퍼폼 → 시장보다 더 오름, 스티프닝 → 장단기 금리차 확대, 플래트닝 → 축소, 숏커버 → 공매도 되사기, 익스포저 → 노출·투자 비중, 포지셔닝 → 자금이 어디에 실렸나, 리스크온/오프 → 위험자산 선호/안전자산 선호, 디커플링 → 따로 움직임, 리레이팅 → 몸값 재평가, 모멘텀 → 가격 흐름의 힘. 전체 목록은 `scripts/us/style.py`의 `PLAIN`이 정본이다.
  2. **옮기면 길어지는 진짜 전문어는 첫 등장에서 한 번 푼다.** 기간프리미엄·기대인플레·디스인플레이션·확산지수·레짐·밸류에이션·컨센서스·가이던스·멀티플·캐리·베타. 괄호든 「즉 ~」든 「~라는 뜻」이든 형식은 자유고, 한 번 풀었으면 그 뒤로는 그냥 쓴다. 예: 「기간프리미엄(만기가 길수록 더 얹어 받는 값)이 올랐습니다」.
  3. **한 문장에 낯선 말을 겹치지 않는다.** 사용자가 든 「브레드스 확산 트리거」가 정확히 이 형태다 — 낱말 하나씩은 풀 수 있어도 셋이 쌓이면 문장이 안 읽힌다. 한 문장에 둘 이상이면 문장을 나누거나 하나는 우리말로 바꾼다.

  `scripts/check_style.py`가 이 셋을 전부 검사한다(`jargon`·`jargon_gloss`·`jargon_stack`). 표·제목·캡션은 대상이 아니다 — 좁은 칸에서는 줄임말이 낫다.
