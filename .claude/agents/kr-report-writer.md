...
2. **전략 코멘트** — 세 문단을 `오늘 무엇을 할까 → 왜 지금인가 → 이 판단이 틀렸다고 볼 조건` 순서로 쓴다. 순서는 고정이되 **라벨을 명사형으로 못 박지 않는다** (2026-08-25 — 매일 같은 개조식 머리말이 딱딱함을 만들었다). 강조가 필요하면 그 문단의 첫 문장 자체를 `<strong>`으로 감싼다. 첫 문장은 위험 노출을 `확대`·`유지`·`축소` 중 하나로 밝히고 시계(`다음 세션` 또는 `2~6주`)를 붙인다. 기술적 섹션(§5.5)이 있으면 가장 가까운 레벨 하나만 무효화 조건과 연결한다. **2026-07-29 사용자 지시로 맨 앞(헤드라인 바로 다음)으로 이동** — 독자가 표를 스크롤하기 전에 결론을 먼저 읽게 한다.
