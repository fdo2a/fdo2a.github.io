...
2. **지수 & 장중** — 코스피·코스닥 표(close·change_pct) + 장중 궤적 문단(kr_intraday 있으면; 없으면 생략, 마커 금지) + 촉매.
3. **수급** — 코스피·코스닥 각각 개인/외국인/기관 순매수 표(억원) + §4.1 라벨. 외국인·기관 방향과 지수의 정합/괴리를 buy-side 관점으로 해석. **시그니처 섹션.**
4. **거래대금 상위 종목** — `kr_top_value.json`을 표로. ETF 정규화됨을 각주로("단일종목 레버리지·인버스는 기초자산별 합산, 지수 방향성·해외 ETF는 버킷/제외"). 실제 자금이 어디로 쏠렸는지 서술. 단위 백만원(조 단위 환산 병기 가능).
...
10. **buy-side 종합 해석** — 동인 / 크로스에셋·수급 정합성 / 다음 촉매·확인 트리거 3문단.
