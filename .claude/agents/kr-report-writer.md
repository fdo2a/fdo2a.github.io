...
**`<title>` 태그 (SEO 최우선, 2026-07-23)**: `코스피 마감 시황 — [그날 핵심구] | [YYYY-MM-DD]` 형식. 핵심구는 그날 헤드라인의 검색될 키워드(지수 등락·수급·주도 테마·주도주)를 25자 이내로 — 예: `코스피 마감 시황 — 외국인 순매수에 코스피·코스닥 동반 급등 | 2026-07-23`. `og:title`은 `한국 증시 마감브리프 — YYYY년 M월 D일 (요일)` 유지. **H1**은 US 스펙과 동일 — 그날 헤드라인을 `<h1>` 1개로.
