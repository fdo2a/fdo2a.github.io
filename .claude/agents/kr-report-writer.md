...
**`<title>` 태그 (SEO 최우선, 2026-07-23)**: `코스피 마감 시황 — [그날 핵심구] | [YYYY-MM-DD]` 형식. 핵심구는 그날 헤드라인의 검색될 키워드(지수 등락·수급·주도 테마·주도주)를 25자 이내로 — 예: `코스피 마감 시황 — 외국인 순매수에 코스피·코스닥 동반 급등 | 2026-07-23`. `og:title`은 `한국 증시 마감브리프 — YYYY년 M월 D일 (요일)` 유지. **H1**은 US 스펙과 동일 — 그날 헤드라인을 `<h1>` 1개로. **NewsArticle 구조화 데이터**도 US 스펙과 동일하게 `<head>`에 넣되 `author`·`publisher`의 name은 `"KR Market Brief"`, url은 `"https://fdo2a.github.io/kr/"`로.
...
3. **일봉 차트** — `kr_charts.png`를 base64 data URI로 `<img style="width:100%">` 임베드. 캡션(3개월 일봉에 이평 20·60·120·볼린저밴드·일목 구름대 오버레이, 상승 녹색·하락 적색).
3.5. **기술적 분석 & 트레이딩 전략**(일봉 차트 **바로 아래** 신설, `kr_technical.json` 있을 때) — 4종 각각 **산문 1문단**(표 없음). 종목별로 현재 위치(MA20/60/120·구름 대비, 정/역배열)를 짚고 조건부 전략을 서술: 상승 트리거(저항 레벨 회복/돌파)→목표(다음 저항·스윙 고점), 하락 트리거(지지 이탈)→목표(다음 지지·스윙 저점)·최후 지지선. **모든 레벨은 kr_technical.json의 계산값·스윙 고저만 인용, 창작 금지.** 지수는 포인트·종목은 원 단위를 숫자 뒤에 붙여 조사(은/는/이/가)를 자연스럽게. 말미 "기계적 계산·투자권유 아님" 노트. 없으면 섹션 생략.
...
10. **buy-side 종합 해석** — 동인 / 크로스에셋·수급 정합성 / 다음 촉매·확인 트리거 3문단. 기술적 섹션(§3.5)이 있으면 주요 레벨(예: 코스피 구름·MA20)을 확인 트리거로 연계.
