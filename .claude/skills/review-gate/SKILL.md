...
`history.jsonl`로 바꾼다. **MLCC 산업 뉴스 로그(`site/thesis/data/mlcc_news.jsonl`)의
`title`·`body`는 사람이 쓴 문장**이므로 큐에 원고로 올라온다. 이건 레포 안의 데이터로
검증되지 않는다 — 각 항목의 `source_url`을 열어 본문의 수치와 대조하고, `confirmed:true`인데
1차 출처(공시·IR·회사 발표)가 아닌 항목을 특히 본다.
...
`publish_commit` 은 조판만 바꾼 커밋(방문 통계 소급 등)을 건너뛰어 본문이 들어온 커밋을
돌려준다. 그래도 근거 `report_date` 가 글 날짜와 다르면(소급 발행, 문장을 고친 재발행)
러너는 그 글을 자동 검토에서 빼고 훅에 `[사람검토] 근거` 로 띄운다 — 그 날짜의 데이터가
담긴 커밋을 직접 찾아 위 스냅샷의 커밋 자리에 넣고 수동으로 검토한다.
...
## 보고 계약 — 근거 없으면 고친 게 아니다 (2026-09-19)

**범위는 「보고 계약 강화」다.** Python 이 아래 세 항목을 검사하게 되는 게 아니다 —
검증 주체는 사람(또는 정정 세션)이고, 이 절은 무엇을 적어야 보고가 완결되는지를 정한다.
설계 근거: `docs/superpowers/specs/2026-09-19-agent-workflow-video-adoption.md`.

「고쳤다」는 보고는 셋을 달고 온다. 하나라도 없으면 완료로 치지 않는다.

| | 무엇을 | 왜 |
|---|---|---|
| ① | 바꾼 **파일:줄** | 「고쳤다」가 어디를 가리키는지 없으면 검증이 불가능하다 |
| ② | 그 변경을 통과시킨 **게이트 명령과 출력 한 줄**, 그리고 **대상 SHA** | 게이트를 돌렸다는 말과 돌린 결과는 다르다 |
| ③ | 뚫렸던 경로를 **재현해 이제 막히는 것을 본 흔적** | 게이트는 한 겹 더 파야 막힌다 — 고친 자리의 이웃 경로가 그대로면 같은 고장이 옆문으로 다시 들어온다 |

**예외 — 이 경우는 세 항목 누락이 아니다.**

- **지적 0건·전부 기각**: 바꾼 본문이 없으므로 ①③ 대신 **「수정 없음과 그 이유」**를 적는다.
  「지적이 0건이어도 `mark`는 한다」는 위 규칙과 충돌하지 않는다.
- **의도한 비정상 종료**: `verify_post.py` 가 「수치가 움직였다」로 exit 1 을 내는 것은
  정상이다(5단계). ② 에는 그 출력과 **왜 그 움직임이 의도된 것인지**를 같이 적는다.
- **문서만 바뀐 변경**: ② 는 해당 문서의 게이트
  (`python3 -m pytest scripts/common/tests/test_agent_docs.py`)로 갈음한다.

## 같은 지적이 세 번째면 멈춘다 (2026-09-19)

검수는 승인이 날 때까지 반복하되, **같은 지적이 3회째 돌아오면 고치기를 멈추고** 지적과
내 반박을 나란히 적어 사용자에게 올린다. 세 번 같은 말이 돌아온다는 건 대개 고칠 대상이
본문이 아니라 **게이트나 계약**이라는 뜻이다.

무인 경로에는 이게 러너에 박혀 있다 — `review/runner.py` 의 `ROUND_CAP`. **같은
발행본**의 자동 정정이 3회 실패하면 더 부르지 않고 훅에 `[사람검토]` 로 띄운다.
날짜가 바뀌어도 풀리지 않는다(하루 상한과 달리 누적이라야 의미가 있다). 그 사이
`remote main changed` 같은 재시도성 실패는 회차로 세지 않는다.

**아래 「위임」 규칙과 헷갈리지 않는다.** 그쪽은 *내가 구현을 못 해내고 있을 때* codex 에
쓰기를 맡기는 것이고, 이쪽은 *같은 발행본의 정정이 계속 실패할 때* 사람에게 넘기는 것이다.
트리거도 다르고 받는 쪽도 다르다.

**한계 — 지적 「단위」가 아니라 발행본 단위다.** 어떤 지적이 반복되는지까지는 세지 않는다.
지적 식별자를 영속화하는 일은 미뤄 뒀다(`plan.md` 2026-09-19 「남은 빚」).
