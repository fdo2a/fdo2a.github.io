...
**2. 스킬을 돌린다.** Skill 도구로 `humanize-korean:humanize-korean`을 호출한다. 대상은 **본문 산문만**이다 — `.doc` 안의 `<p>` 문단들. 표 셀·숫자·티커·섹션 제목·`data-*` 표식·장중 수급 앵커 표와 기술적 레벨는 **손대지 않는다**고 프롬프트에 명시한다.
